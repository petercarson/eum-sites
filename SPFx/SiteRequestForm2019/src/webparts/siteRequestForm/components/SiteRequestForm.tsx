...
export default class SiteRequestForm extends React.Component<ISiteRequestFormProps, ISiteRequestFormState> {
  ...
  public render() {
    let getDivisions: boolean = (!this.state.hasError && !this.state.divisionsLoaded);
  ...
  private GetDivisions(): void {
    sp.web.lists
      ...
      .then((items: IDivisionListItem[]): void => {
        ...
        if (items.length === 1) {
          this.setState({ isLoading: false, divisions: items, divisionsLoaded: true, selectedDivision: items[0].Id });
          ...
          if (filteredItems.length > 0) {
            ...
            this.setState({ isLoading: false, divisions: items, divisionsLoaded: true, selectedDivision: this.PreselectedDivision.Id });
          } else {
    ...
    return (
      <div id="DivisionDropdown">
        <Dropdown
          title="EUMDivisionId"
          id="EUMDivisionId"
          label={strings.DivsionDropdownLabel}
          placeHolder={strings.DivsionDropdownPlaceholderText}
          required={true}
          options={this.state.divisions.map(division => ({ key: division.Id, text: division.Title }))}
          onChanged={(item) => this.setState({ isLoading: true, siteTemplatesLoaded: false, selectedSiteTemplate: null, selectedDivision: item.key.toString() })}
    ...
    return (
      <div id="SiteTemplateDropdownSection">
        <Dropdown
          ...
          id="EUMSiteTemplate"
          label={strings.SiteTemplateDropdownLabel}
      ...
      this.state.fields.map((spField) => {
        ...
        <div className={styles.formButtonsContainer}>
          <PrimaryButton
            ...
            disabled={this.state.isSaving || this.state.isLoading || this.state.aliasValidating || !this.state.aliasIsValid}
          />
          <DefaultButton
            text={strings.CancelButtonText}
            ariaDescription={strings.CancelButtonText}
            onClick={() => this.ClearSiteRequest()}
            disabled={this.state.isSaving || this.state.isLoading || this.state.aliasValidating}
          />
        </div>
      )
  ...
  private async ValidateSiteUrlField(value: any, required: boolean): Promise<string> {
    if (value) {
      ...
      if (!(siteUrl.toLocaleLowerCase().indexOf(this.props.tenantUrl.toLocaleLowerCase()) > 0)) {
  ...
  private async SaveTitleFieldValue(fieldInternalName: string, newValue: any, fieldType: string) {
    ...
    if (newValue) {
      ...
      if (!this.ModernTeamSite) {
        generatedAlias = `/sites/${generatedAlias}`;
      }
  ...
  private SaveSiteRequestWithSharePoint(): void {
    ...
    if (this.ModernTeamSite) {
      postData['EUMAlias'] = this.state.alias;
    } else {
      postData['EUMSiteURL'] = this.state.alias;
    }

    let fieldsValid: boolean = this.ValidateSubmit(postData);
    if (!fieldsValid || !this.state.aliasIsValid) {
